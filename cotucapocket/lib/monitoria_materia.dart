...
class _MonitorScheduleTableState extends State<MonitorScheduleTable> {
  ...
  Color _gerarCor(int index) {
    ...
    double lightness =
        0.4; // Mantém a luminosidade baixa para evitar cores claras
                        ...
                        child: Center(
                          child: Text(
                            ...
                            style: TextStyle(
                              fontSize: 11,
                              color:
                                  dia == diaAtual ? Colors.red : Colors.black,
                            ),
                          ),
                        ),
                      ),
  ...
  Widget _buildMonitoresText(String monitoresNoHorario) {
    ...
    for (int i = 0; i < monitores.length; i++) {
      ...
      if (nomeMonitor.isNotEmpty) {
        Color cor = _coresMap[nomeMonitor] ??
            Colors.black; // Cor padrão se não encontrado
        spans.add(
          TextSpan(
            text: monitor +
                (i < monitores.length - 1
                    ? ', '
                    : ''), // Usa o monitor completo, incluindo a sala
      ...
      if (currentStart == null) {
        ...
        if ((currentTimeParsed.isAtSameMomentAs(lastEndTime) ||
            (currentTimeParsed.isAfter(lastEndTime) &&
                currentTimeParsed.isBefore(lastEndTime.add(Duration(
                    minutes: 61))) && 
                (currentLocation == location ||
